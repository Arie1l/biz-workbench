...
(function() {
  ...
  function toggleTask(id) {
    Store.toggleTask(id);
    // 自动同步：任务完成时更新关联订单的对应步骤（支持一个任务匹配多个步骤）
    var tasks = Store.getTasks();
    var task = tasks.find(function(t) { return t.id === id; });
    var results = null;
    if (task && task.completed && task.relatedOrder) {
      results = Store.syncTaskToOrderStep(task);
      if (results && results.length > 0) {
        var stepNames = results.map(function(r) { return r.stepName; }).join('、');
        showToast('已同步「' + results[0].orderNumber + '」→ ' + stepNames, 'success');
      }
    }
    renderTasks();
    if (currentPage === 'dashboard') renderDashboard();
    // 如果正在查看该订单详情，刷新视图
    if (results && results.length > 0 && currentOrderId === results[0].orderId) {
      viewOrder(currentOrderId);
    }
  }
  ...
  function renderDailyGrid(year, month, monthStr, monthTasks, orders) {
    ...
    for (var di = 0; di < dateList.length; di++) {
      ...
      for (var pi = 0; pi < people.length; pi++) {
        var personItems = cellData[dateStr][people[pi]] || [];
        for (var ii = 0; ii < personItems.length; ii++) {
          allItems.push(personItems[ii]);
        }
      }
      flatByDate[dateStr] = allItems;
      maxItems = Math.max(maxItems, allItems.length);
    }

    // 构建表格：列=日期，行=任务序号（第1个任务在第1行，第2个在第2行...）
    ...
    if (maxItems === 0) {
      html += '<tr><td colspan="' + dateList.length + '" style="text-align:center;color:#9ca3af;padding:24px">本月暂无记录</td></tr>';
    } else {
      for (var ri = 0; ri < maxItems; ri++) {
        html += '<tr>';
        for (var di2 = 0; di2 < dateList.length; di2++) {
          var items = flatByDate[dateList[di2]];
          if (ri < items.length) {
            var item = items[ri];
            html += '<td>';
            html += '<div class="daily-grid-item">';
            html += '<span class="item-num">' + (ri + 1) + '</span>';
  ...
  function exportDailyGrid() {
    ...
    for (var ri = 0; ri < maxItems; ri++) {
      ...
      for (var di2 = 0; di2 < dateList.length; di2++) {
        ...
        if (ri < items.length) {
          var item = items[ri];
          row.push(item.statusText + ' ' + item.content + ' [销售:' + item.sp + ']');
        } else {
          row.push('');
        }
      }
